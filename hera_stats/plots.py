...
class plots():
    ...
    def plot_spectra_errs(self,n=1,fig=None,show_groups=False,savefile=None):
        ...
        # Create larger, upper subplot for spectra
    ...
    def hist_2d(self,plottype="varsum",fig=None,ax=None,ybins=40,display_stats=True,
                vmax=None,returned=False):
        """Plots heatmap of the 
        """
        ...
        ax.set_title("Jackknife Data",fontsize=16)
        ...
        if vmax == None:
            vmax = np.max(hist[0]/len(data))/2
        c = ax.pcolor(hist[1],hist[2],hist[0].T/len(data),vmax=vmax)
    ...
    def plot_anderson(self,ax=None, method="varsum"):
        ...
        ax.set_xlabel("Delay (ns)")
        ax.set_title("Anderson Darling Test by Delay Bin")
        ax.set_ylim(0,1.15)
        ax.grid(True)
    ...
    def plot_av_std(self,ax=None,method="varsum"):
        ...
        ax.set_title("Standard Deviation and Average")
        ax.set_xlabel("Delay (ns)")
        ax.grid(True)
